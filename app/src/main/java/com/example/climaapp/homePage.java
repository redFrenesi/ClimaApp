...
public class homePage extends AppCompatActivity implements LocationListener {
    ...
    private final int TiempoConstante = 60;
            ...
            if (tiempoTick >= TiempoConstante / 2) {
                ...
                if (!start) {
                    if (idGlobal <= 0) {
                        ...
                        Log.e("ubicacion local=", idGlobal + "/" + latitudGlobal + "/" + longitudGlobal + "/" + Ubicacion);
                        ApiRest2(latitudLocal, longitudLocal, idGlobal, Ubicacion);
                    }
                }
            }
    ...
    public void onLocationChanged(@NonNull Location location) {
        ...
        Log.e("onCreate: ", location.getLatitude() + "," + +location.getLongitude());
                ...
                .addOnSuccessListener(new OnSuccessListener<Location>() {
                    ...
                    public void onSuccess(Location location) {
                        ...
                        if (location != null) {
                            ...
                            Log.e("metodo 1 ", latitudLocal + "," + longitudLocal);
                            Log.e("ubicacion local=", idGlobal + "/" + latitudGlobal + "/" + longitudGlobal + "/" + Ubicacion);
    ...
    private void ApiRest2(double latitud, double longitud, int id, String pais) {
        if (latitud == 0.0 && longitud == 0.0 && start == false) {
            Log.e("consultando datos", "Saliendo no hay cordenadas");
            offline(Ubicacion);
            return;
        }
        Log.e("consultando datos", "datos siendo consultados");
        String url = "https://api.openweathermap.org/data/2.5/onecall?lat=" + latitud + "&lon=" + longitud + "&exclude=minutely,hourly&appid=5621c00c5d57dd7ce40821da31af78ee";
                        ...
                        try {
                            for (int i = 0; i < array.length(); i++) {
                                ...
                                if (pais.get("country").equals((lista.get(position)))) {
                                    ...
                                    String nameP = lista.get(position);
                                    latitudGlobal = latitudP;
                                    longitudGlobal = longitudP;
                                    Ubicacion = nameP;
                                    idGlobal = idP;
                                    Log.e("ubicacion remota=", idGlobal + "/" + latitudGlobal + "/" + longitudGlobal + "/" + Ubicacion);
    ...
    public void offline(String pais) {
        ...
        for (int i = 0; i < ListaPaises.size(); i++) {
            if (ListaPaises.get(i).getNombre().contains(pais)) {
                ...
                contenedor.removeAllViews();
                tempActual.setText(tmp1 + "°");
                timeZone.setText(ZonaHoraria);
                agregarCards(tmp1 + "°", horas, addDays(0), true);
                agregarCards(tmp2 + "°", horas, addDays(1), false);
                agregarCards(tmp3 + "°", horas, addDays(2), false);
                agregarCards(tmp4 + "°", horas, addDays(3), false);
                agregarCards(tmp5 + "°", horas, addDays(4), false);
                agregarCards(tmp6 + "°", horas, addDays(5), false);
                agregarCards(tmp7 + "°", horas, addDays(6), false);
                break;
